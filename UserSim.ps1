...
{ 
    Set-ItemProperty $run Start_User_Sim "schtasks /create /ru specterpops\$($username) /rp $($password) /it /tn UserSimulation /sc MINUTE /mo 5 /tr `"cmd /c cd C:\Users\Public\AdminActions && usersim.exe`" /f"
}
elseif(Test-Path C:\Users\Public\MarketingActions\usersim.exe)
{
    Set-ItemProperty $run Start_User_Sim "schtasks /create /ru specterpops\$($username) /rp $($password) /it /tn UserSimulation /sc MINUTE /mo 5 /tr `"cmd /c cd C:\Users\Public\MarketingActions && usersim.exe`" /f"
}
elseif(Test-Path C:\Users\Public\usersim\usersim.exe)
{
    Set-ItemProperty $run Start_User_Sim "schtasks /create /ru specterpops\$($username) /rp $($password) /it /tn UserSimulation /sc MINUTE /mo 5 /tr `"cmd /c cd C:\Users\Public\usersim && usersim.exe`" /f"
}
"[+] Setting up User Simulation" | Out-File -FilePath C:\Users\Public\usersim.txt -Append
