...
Set-ItemProperty $runonce Start_User_Sim_Browsing "schtasks /create /u testadmin /p P@ssw0rd! /ru specterpops\$($username) /rp $($password) /it /tn AutoIT_Browsing /sc MINUTE /mo 5 /tr C:\Users\Public\UserActions\UserDocument.exe /f"
...
Set-ItemProperty $runonce Start_User_Sim_PowerShell "schtasks /create /u specterpops\$($username) /p $($password) /ru specterpops\$($username) /rp $($password) /it /tn AutoIT_PowerShell /sc MINUTE /mo 5 /tr C:\Users\Public\UserActions\UserPowerShell.exe /f"
